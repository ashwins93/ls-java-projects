...
public class CountriesController {
  ...
  public ArrayList<Country> getAllCountries() {
    countryList.sort(new NameComparator());
    return countryList;
  }

  @GetMapping("/names/start/{startLetter}")
  public ArrayList<Country> getCountriesStartWith(@PathVariable String startLetter) {
    ArrayList<Country> result = new ArrayList<>(countryList.size());
    countryList.stream().filter(country -> country.getName().toLowerCase().startsWith(startLetter))
        .sorted(new NameComparator()).forEach(country -> result.add(country));

    return result;
  }

  @GetMapping("/names/size/{size}")
  public ArrayList<Country> getCountriesWithSize(@PathVariable int size) {
    ArrayList<Country> result = new ArrayList<>(countryList.size());
    countryList.stream().filter(country -> country.getName().length() >= size).sorted(new NameComparator())
  ...
  public ArrayList<Country> getCountriesWithAge(@PathVariable int age) {
    ArrayList<Country> result = new ArrayList<>(countryList.size());
    countryList.stream().filter(country -> country.getMedianAge() >= age).forEach(country -> result.add(country));

    return result;
  }

  @GetMapping("/age/min")
  public Country getCountryWithMinAge() {
